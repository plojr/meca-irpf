...
@SpringBootTest
public class TestesGrupamento01 {
	...
	public void teste01() {
		try {
			...
			Grupamento grupamento1 = new Grupamento(ticker1, LocalDate.of(2020, 1, 3), 10);
			Grupamento grupamento2 = new Grupamento(ticker2, LocalDate.of(2020, 2, 3), 5);
			List<EventoExtraordinario> eventos = new ArrayList<>();
			eventos.add(grupamento1);
			eventos.add(grupamento2);
			...
			for(ItemCarteira ic: itensCarteira) {
				if(ic.getTicker().equals("itsa4")) {
					...
					assertEquals(10, ic.getQuantidade());
				}
				else if(ic.getTicker().equals("bbdc3")) {
					assertEquals(2000.60, ic.getCustoTotal(), EPSILON);
					assertEquals(20, ic.getQuantidade());
				}
			}
		} catch(Exception e) {
			System.err.println("TesteGrupamento01.teste01(): " + e.getMessage());
		}
	}
